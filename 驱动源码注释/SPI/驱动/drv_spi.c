...
static rt_uint32_t spixfer(struct rt_spi_device *device, struct rt_spi_message *message)
{
    ...
    rt_uint8_t *recv_buf;       /* 接收buffer */
    const rt_uint8_t *send_buf; /* 发送buffer */
    ...
    recv_buf = message->recv_buf;/* 接收buffer */
    send_buf = message->send_buf;/* 发送buffer */
...
static int rt_hw_spi_bus_init(void)
{
    ...
    {
        spi_bus_obj[i].config = &spi_config[i];                  /* 初始化SPI总线配置 */
        spi_bus_obj[i].spi_bus.parent.user_data = &spi_config[i];/* 初始化SPI设备 */
        spi_bus_obj[i].handle.Instance = spi_config[i].Instance; /* 初始化SPI总线寄存器基地址 */
        /* 将SPI总线设备注册到设备管理器  */
...
rt_err_t rt_hw_spi_device_attach(const char *bus_name, const char *device_name, GPIO_TypeDef *cs_gpiox, uint16_t cs_gpio_pin)
{
    ...
    rt_err_t result;
    struct rt_spi_device *spi_device;/* SPI设备（挂） */
    struct ch32_hw_spi_cs *cs_pin;   /* SPI设备（挂）CS */
