...
{
    const void *send_buf;        /* 发送buffer */
    void *recv_buf;              /* 接收buffer */
    rt_size_t length;            /* 数据长度  */
    struct rt_spi_message *next; /*  指向下一个结构体的指针 */

    unsigned cs_take    : 1;     /* 总线获取标志 */
    unsigned cs_release : 1;     /* 总线释放*/
};
...
struct rt_spi_bus
{
    struct rt_device parent;        /* SPI总线设备的爸爸 */
    rt_uint8_t mode;                /* SPI总线模式 */
    const struct rt_spi_ops *ops;   /* SPI总线方法 */

    struct rt_mutex lock;           /* SPI总线互斥锁 */
    struct rt_spi_device *owner;    /* SPI设备（挂）*/
};

/**
 * SPI operators
 */
struct rt_spi_ops /* 总线设备方法（挂）*/
{
...
{
    struct rt_device parent; /* 设备的爸爸 */
    struct rt_spi_bus *bus;  /* 设备总线 */
